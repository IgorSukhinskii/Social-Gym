...
Layout files should match the name of the Android components that they are intended for but prefixed with their namespace. Examples:

| Component        | Class Name                         | Layout Name                         |
| ---------------- | ---------------------------------- | ----------------------------------- |
| Activity         | `UserProfileActivity`              | `user_profile_activity.xml`         |
| Fragment         | `user_profile.SignUpFragment`      | `user_profile_sign_up_fragment.xml` |
| Dialog           | `user_profile.dialogs.ChangePasswordDialog`             | `user_profile_change_password_dialog.xml`        |
| AdapterView item | ---  the adapter is used in `user_profile.PersonFragment`                              | `user_profile_person_item.xml`                   |
| Partial layout   | ---                                | `stats_bar_partial.xml`             |

A slightly different case is when we are creating a layout that is going to be inflated by an `Adapter`, e.g to populate a `ListView`. In this case, the name of the layout should end with `_item`.

Note that there are cases where these rules will not be possible to apply. For example, when creating layout files that are intended to be part of other layouts. In this case you should use the suffix `_partial`.
...
```java
Picasso.with(context).load("http://example.com/images/example.jpg").into(imageView);
```

```java
Picasso.with(context)
        .load("http://example.com/images/example.jpg")
        .into(imageView);
```
...
```java
loadPicture(context, "http://example.com/images/example.jpg", mImageViewProfilePicture, clickListener, "Title of the picture");
```
